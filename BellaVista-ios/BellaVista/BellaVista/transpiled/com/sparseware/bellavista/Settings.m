...
- (void)onNetworkServersDownloadWithNSString:(NSString *)eventName
  ...
  @try {
    (void) [((RAREWindowViewer *) nil_chk(w)) getContentAsJSONWithId:url withRAREiFunctionCallback:[[CCPBVSettings_$3 alloc] initWithCCPBVSettings:self withRAREWindowViewer:w withRAREaListViewer:lv]];
    [w showWaitCursor];
  }
  @catch (JavaNetMalformedURLException *e) {
    [CCPBVUtils handleErrorWithJavaLangThrowable:e];
...
- (void)onServersChangeWithNSString:(NSString *)eventName
  ...
  [((id<RAREiWidget>) nil_chk([fv getWidgetWithNSString:@"deleteButton"])) setEnabledWithBoolean:n != -1];
  [((id<RAREiWidget>) nil_chk([fv getWidgetWithNSString:@"upButton"])) setEnabledWithBoolean:n > -1 && n > 0];
  [((id<RAREiWidget>) nil_chk([fv getWidgetWithNSString:@"downButton"])) setEnabledWithBoolean:n > -1 && n < [lv size] - 1];
}
...
- (void)onSettingsUnloadWithNSString:(NSString *)eventName
                     withRAREiWidget:(id<RAREiWidget>)widget
             withJavaUtilEventObject:(JavaUtilEventObject *)event {
  [((CCPBVSettings_AppPreferences *) nil_chk(preferences_)) update];
}
...
- (void)populateServersWithRAREaListViewer:(RAREaListViewer *)lv {
  ...
  for (CCPBVSettings_Server * __strong s in nil_chk(servers_)) {
    [lv addExWithRARERenderableDataItem:[[RARERenderableDataItem alloc] initWithNSString:[((CCPBVSettings_Server *) nil_chk(s)) getName] withId:s withRAREiPlatformIcon:nil]];
  }
  [lv refreshItems];
}
...
- (id<JavaUtilList>)getServers {
  if (servers_ == nil) {
    ...
    @try {
      ...
      JavaUtilHashSet *set = [[JavaUtilHashSet alloc] initWithInt:len > 1 ? len : 1];
      for (int i = 0; i < len; i++) {
        ...
        if ((s != nil) && ([s sequenceLength] > 0)) {
          @try {
            ...
            if ([server isValid] && [set addWithId:[server toHashKey]]) {
...
- (void)finishedWithBoolean:(BOOL)canceled
  ...
  if ([returnValue isKindOfClass:[JavaLangThrowable class]]) {
    ...
    @try {
      ...
      for (id __strong s in nil_chk(a)) {
        CCPBVSettings_Server *server = [[CCPBVSettings_Server alloc] initWithRAREUTJSONObject:(RAREUTJSONObject *) check_class_cast(s, [RAREUTJSONObject class])];
        if (![((JavaUtilHashSet *) nil_chk(set)) containsWithId:[server toHashKey]]) {
      ...
      if (count == 0) {
        [val$w_ alertWithId:[val$w_ getStringWithNSString:@"bv.text.settings.no_new_servers"]];
      }
      else {
    ...
    @catch (JavaLangException *e) {
      [val$w_ alertWithId:[val$w_ getStringWithNSString:@"bv.text.settings.invalid_network_servers_data"]];
      [RAREPlatform ignoreExceptionWithNSString:nil withJavaLangThrowable:e];
    }
  }
}
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _CCPBVSettings_$3 = { "$3", "com.sparseware.bellavista", "Settings", 0x8000, 0, NULL, 3, fields, 0, NULL};
