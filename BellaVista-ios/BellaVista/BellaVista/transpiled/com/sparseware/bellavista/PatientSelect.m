...
static id<JavaLangCharSequence> CCPBVPatientSelect_actionBarTitle_;
static id<RAREiPlatformIcon> CCPBVPatientSelect_actionBarIcon_;
...
+ (id<JavaLangCharSequence>)actionBarTitle {
  return CCPBVPatientSelect_actionBarTitle_;
}

+ (void)setActionBarTitle:(id<JavaLangCharSequence>)actionBarTitle {
  CCPBVPatientSelect_actionBarTitle_ = actionBarTitle;
}

+ (id<RAREiPlatformIcon>)actionBarIcon {
  return CCPBVPatientSelect_actionBarIcon_;
}

+ (void)setActionBarIcon:(id<RAREiPlatformIcon>)actionBarIcon {
  CCPBVPatientSelect_actionBarIcon_ = actionBarIcon;
}
...
- (void)stateChangedWithJavaUtilEventObject:(JavaUtilEventObject *)e {
  ...
  if ([((CCPBVaPatientLocator_LocatorChangeEvent *) nil_chk(ce)) getChangeType] == [CCPBVaPatientLocator_LocatorChangeTypeEnum ACCESS_DENIED]) {
    [self stopListeningForNearbyPatients];
    RAREWindowViewer *w = [RAREPlatform getWindowViewer];
    [w alertWithId:[((RAREWindowViewer *) nil_chk(w)) getStringWithNSString:@"bv.text.location_service_denied_access"]];
    return;
  }
...
+ (void)resetActionBar {
  if (CCPBVPatientSelect_actionBarTitle_ != nil) {
    RAREActionBar *ab = [((RAREWindowViewer *) nil_chk([RAREPlatform getWindowViewer])) getActionBar];
    [((RAREActionBar *) nil_chk(ab)) setTitleWithJavaLangCharSequence:CCPBVPatientSelect_actionBarTitle_];
    [ab setTitleIconWithRAREiPlatformIcon:CCPBVPatientSelect_actionBarIcon_];
  }
}
...
+ (void)updateActionBar {
  if ([RAREUIScreen isMediumScreen]) {
    RAREActionBar *ab = [((RAREWindowViewer *) nil_chk([RAREPlatform getWindowViewer])) getActionBar];
    if (CCPBVPatientSelect_actionBarTitle_ == nil) {
      CCPBVPatientSelect_actionBarTitle_ = [((id<RAREiActionComponent>) nil_chk([((RAREActionBar *) nil_chk(ab)) getTitleComponent])) getText];
      CCPBVPatientSelect_actionBarIcon_ = [((id<RAREiActionComponent>) nil_chk([ab getTitleComponent])) getIcon];
    }
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _CCPBVPatientSelect = { "PatientSelect", "com.sparseware.bellavista", NULL, 0x1, 17, methods, 34, fields, 0, NULL};
