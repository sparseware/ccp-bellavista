...
static CCPBVPatientSelect_PatientIcon * CCPBVPatientSelect_noPhotoIcon_;
...
- (id)init {
  if (self = [super init]) {
    pager_ = [[CCPBVListPager alloc] init];
    ...
    alwaysShowSearchFirst_ = [info optBooleanWithNSString:@"alwaysShowSearchFirst" withBoolean:YES];
    genderSearchSupported_ = [info optBooleanWithNSString:@"genderSearchSupported" withBoolean:YES];
...
- (void)onIdentifierLabelShownWithNSString:(NSString *)eventName
  ...
  if ([RAREPlatform isAndroid]) {
    ...
    [((id<RAREiWidget>) nil_chk([((id<RAREiFormViewer>) nil_chk([((id<RAREiWidget>) nil_chk(widget)) getFormViewer])) getWidgetWithNSString:@"identifier"])) requestFocus];
  }
}
...
- (void)onListsBoxFinishLoadingWithNSString:(NSString *)eventName
  ...
  if ([((RAREaListViewer *) nil_chk(lb)) isEmpty]) {
    RAREWindowViewer *w = [RAREPlatform getWindowViewer];
    CCPBVCollectionManager_PatientList *pl = (CCPBVCollectionManager_PatientList *) check_class_cast([lb getLinkedData], [CCPBVCollectionManager_PatientList class]);
    [((RARERenderableDataItem *) nil_chk(noPatientListLoaded_)) setValueWithId:[((RAREWindowViewer *) nil_chk(w)) getStringWithNSString:@"bv.format.no_lists" withNSObjectArray:[IOSObjectArray arrayWithObjects:(id[]){ ((CCPBVCollectionManager_PatientList *) nil_chk(pl))->title_ } count:1 type:[IOSClass classWithClass:[NSObject class]]]]];
...
- (void)onNextOrPreviousPageWithNSString:(NSString *)eventName
  ...
  if (next) {
    link = [((CCPBVListPager *) nil_chk(pager_)) next];
  }
  else {
    link = [((CCPBVListPager *) nil_chk(pager_)) previous];
  }
  if (link == nil) {
...
- (void)onPatientSearchWithNSString:(NSString *)eventName
  ...
  [self stopListeningForNearbyPatients];
  RAREWindowViewer *w = [RAREPlatform getWindowViewer];
  id<RAREiContainer> form = [((id<RAREiWidget>) nil_chk(widget)) getFormViewer];
  RARETableViewer *table = patientsTable_;
  [((RARETableViewer *) nil_chk(table)) clear];
  [self clearPreview];
  [((CCPBVListPager *) nil_chk(pager_)) clear];
  NSString *id_ = [((id<RAREiWidget>) nil_chk([((id<RAREiContainer>) nil_chk(form)) getWidgetWithNSString:@"identifier"])) getValueAsString];
...
- (void)onSearchFormConfigureWithNSString:(NSString *)eventName
  ...
  if (!genderSearchSupported_) {
    id<RAREiWidget> gender = [fv getWidgetWithNSString:@"gender"];
    if (gender != nil) {
      [gender setVisibleWithBoolean:NO];
    }
    gender = [fv getWidgetWithNSString:@"genderLabel"];
    if (gender != nil) {
      [gender setVisibleWithBoolean:NO];
    }
  }
  ...
  if (cardStack) {
    ...
    if (![RAREUIScreen isSmallScreen]) {
      ...
      if (pb2 != nil) {
        RAREUIImage *sprite = [((id<RAREiPlatformAppContext>) nil_chk([RAREPlatform getAppContext])) getResourceAsImageWithNSString:@"bv.icon.beacon.sprite"];
        RAREUISpriteIcon *icon = [[RAREUISpriteIcon alloc] initWithRAREUIImage:sprite];
        [icon setFrozenWithBoolean:[RAREUIScreen isMediumScreen]];
        [pb2 setIconWithRAREiPlatformIcon:icon];
...
- (void)onSelectPatientWithNSString:(NSString *)eventName
  ...
  id<RAREiPlatformIcon> icon = [((RARERenderableDataItem *) nil_chk([row getWithInt:0])) getIcon];
  id<RAREiFunctionCallback> cb = [[CCPBVPatientSelect_$3 alloc] initWithRAREiPlatformIcon:icon];
...
- (void)onTabPaneCreatedWithNSString:(NSString *)eventName
             ...
             withJavaUtilEventObject:(JavaUtilEventObject *)event {
  if ([RAREUIScreen isLargeScreen]) {
    if (alwaysShowSearchFirst_ || ((barcodeReader_ != nil) && [barcodeReader_ isReaderAvailable]) || ((patientLocator_ != nil) && [patientLocator_ isNearbyPatientsSupported])) {
...
- (void)showFormForButtonWithNSString:(NSString *)eventName
  ...
  [self stopListeningForNearbyPatients];
  [self clearPreview];
  NSString *name = [((id<RAREiWidget>) nil_chk(widget)) getName];
  ...
  if ([((NSString *) nil_chk(name)) isEqual:@"listsButton"] && [((RAREStackPaneViewer *) nil_chk(sp)) isViewerLoadedWithNSString:name]) {
    ...
    if ((pl != nil) && [pl isContainsPatients]) {
      [cb clearSelection];
      RAREaListViewer *lb = (RAREaListViewer *) check_class_cast([c getWidgetWithNSString:@"listsBox"], [RAREaListViewer class]);
      if (lb != nil) {
        [lb clear];
      }
    }
...
- (void)clearPreview {
  ...
  id<RAREiContainer> fv = (id<RAREiContainer>) check_protocol_cast([((RAREWindowViewer *) nil_chk(w)) getViewerWithNSString:@"patientSelectionForm"], @protocol(RAREiContainer));
  if (fv != nil) {
    [((id<RAREiWidget>) nil_chk([fv getWidgetWithNSString:@"patient"])) setValueWithId:[w getStringWithNSString:@"bv.text.no_patient"]];
...
- (void)loadPatientListWithRARETableViewer:(RARETableViewer *)table
  ...
  RAREWindowViewer *w = [RAREPlatform getWindowViewer];
  RAREaWorkerTask *task = [[CCPBVPatientSelect_$6 alloc] initWithCCPBVPatientSelect:self withRAREActionLink:link withRARETableViewer:table withNSString:filter withNSString:gender withRAREWindowViewer:w];
  ...
  [w showWaitCursorWithRAREUTiCancelable:task];
}
...
- (void)populatePreviewWithRARERenderableDataItem:(RARERenderableDataItem *)row {
  ...
  if ((loc == nil) || ([@"" isEqual:loc])) {
    loc = [w getStringWithNSString:@"bv.text.unknownLocation"];
  }
  ...
  if (field != nil) {
    if ([((NSString *) nil_chk(s)) sequenceLength] > 0) {
      [field setValueWithId:[w getStringWithNSString:@"bv.format.location_rm_bed" withNSObjectArray:[IOSObjectArray arrayWithObjects:(id[]){ loc, s } count:2 type:[IOSClass classWithClass:[NSObject class]]]]];
    }
    else {
      [field setValueWithId:loc];
    }
  ...
  RAREImagePaneViewer *ip = (RAREImagePaneViewer *) check_class_cast([fv getWidgetWithNSString:@"photo"], [RAREImagePaneViewer class]);
  if (ip != nil) {
    CCPBVPatientSelect_PatientIcon *icon = (CCPBVPatientSelect_PatientIcon *) check_class_cast([((RARERenderableDataItem *) nil_chk([row getWithInt:0])) getIcon], [CCPBVPatientSelect_PatientIcon class]);
    RAREUIImage *img = (icon == nil) ? nil : [icon getImage];
    [ip setImageWithRAREUIImage:img];
  }
}
...
- (id<JavaUtilList>)processPatientsListWithRARETableViewer:(RARETableViewer *)widget
  ...
  [((RARETableViewer *) nil_chk(widget)) clear];
  ...
  for (int i = len - 1; i >= 0; i--) {
    ...
    if (checkForSignal) {
      [self updateSignalIconWithRARERenderableDataItem:row];
    }
    ...
    if ([row isEnabled]) {
      [((RARERenderableDataItem *) nil_chk([row getWithInt:0])) setIconWithRAREiPlatformIcon:[CCPBVPatientSelect getThimbnailWithRAREaWidget:widget withNSString:(NSString *) check_class_cast([((RARERenderableDataItem *) nil_chk([row getWithInt:CCPBVPatientSelect_PHOTO])) getValue], [NSString class])]];
    }
  }
  return rows;
}
...
- (void)showPreviewWithRARETableViewer:(RARETableViewer *)widget {
  ...
  id<RAREiWidget> patient = [((id<RAREiFormViewer>) nil_chk([widget getFormViewer])) getWidgetWithNSString:@"patient"];
  if (patient != nil) {
    if ([patient getLinkedData] == row) {
      return;
    }
  }
  [((id<RAREiWidget>) nil_chk(patient)) setEnabledWithBoolean:YES];
  [patient setLinkedDataWithId:row];
  [self populatePreviewWithRARERenderableDataItem:row];
}
...
- (void)updateSignalIconWithRARERenderableDataItem:(RARERenderableDataItem *)row {
  RARERenderableDataItem *item = [((RARERenderableDataItem *) nil_chk(row)) getItemExWithInt:CCPBVPatientSelect_SIGNAL];
  if (item != nil) {
...
- (void)patientsTableUpdatedWithRARETableViewer:(RARETableViewer *)table
  ...
  if ([CCPBVUtils isCardStack]) {
    ...
    [((CCPBVListPager *) nil_chk(pager_)) addWithRAREActionLink:link];
    pn = [((RAREActionLink *) nil_chk(link)) getPagingNext];
    if (pn != nil) {
      [pager_ setNextWithRAREActionLink:[CCPBVListPager createPagingLinkWithRAREActionLink:link withNSString:pn]];
    }
...
+ (void)changePatientWithRAREiWidget:(id<RAREiWidget>)context
  ...
  RAREWindowViewer *w = [RAREPlatform getWindowViewer];
  id<RAREiFunctionCallback> cb = [[CCPBVPatientSelect_$8 alloc] initWithRAREiWidget:context withRAREiPlatformAppContext:app withRAREWindowViewer:w withCCPBVActionPath:path];
...
+ (id)getLinkedValueWithNSString:(NSString *)name
  ...
  id obj = [((RAREUTJSONObject *) nil_chk(o)) optWithNSString:name];
  if ([obj isKindOfClass:[RAREUTJSONObject class]]) {
    obj = [((RAREUTJSONObject *) check_class_cast(obj, [RAREUTJSONObject class])) optWithNSString:@"value"];
  }
  return obj;
}

+ (id<RAREiPlatformIcon>)getThimbnailWithRAREaWidget:(RAREaWidget *)context
  ...
  if (link != nil) {
    ...
    @try {
      icon = [((RAREaWidget *) nil_chk(context)) getIconWithJavaNetURL:[link getURLWithRAREUTiURLResolver:context] withNSString:s withFloat:1];
      if (([(id) icon isKindOfClass:[RAREUIImageIcon class]]) && [((RAREUIImageIcon *) check_class_cast(icon, [RAREUIImageIcon class])) getImage] != [RAREaUIImageIcon getBrokenImage]) {
        icon = [[CCPBVPatientSelect_PatientIcon alloc] initWithRAREiPlatformIcon:icon];
        return icon;
      }
    }
...
+ (void)processPatientDataWithRAREUTJSONObject:(RAREUTJSONObject *)patient {
  ...
  NSString *s;
  RAREUTCharArray *ca = [[RAREUTCharArray alloc] init];
  name = [((RAREUTJSONObject *) nil_chk(patient)) getStringWithNSString:@"name"];
  name = [((RAREUTCharArray *) nil_chk([((RAREUTCharArray *) nil_chk([ca setWithNSString:name])) toTitleCase])) description];
  (void) [app putDataWithId:@"pt_name" withId:name];
  (void) [app putDataWithId:@"pt_mrn" withId:mrn = [patient optStringWithNSString:@"mrn"]];
  if ((mrn != nil) && ([mrn sequenceLength] > 0)) {
  ...
  (void) [app putDataWithId:@"pt_name_age_sex" withId:[((RAREWindowViewer *) nil_chk(w)) getStringWithNSString:@"bv.format.name_age_gender" withNSObjectArray:[IOSObjectArray arrayWithObjects:(id[]){ name, [app getDataWithId:@"pt_age_sex"] } count:2 type:[IOSClass classWithClass:[NSObject class]]]]];
  ...
  s = @"";
  if (date != nil) {
    s = [RAREFunctions convertDateWithId:date];
  }
  ...
  NSString *provider = (NSString *) check_class_cast([CCPBVPatientSelect getLinkedValueWithNSString:@"provider" withRAREUTJSONObject:patient], [NSString class]);
  s = (NSString *) check_class_cast([CCPBVPatientSelect getLinkedValueWithNSString:@"attending" withRAREUTJSONObject:patient], [NSString class]);
  ...
  obj = [CCPBVPatientSelect getLinkedValueWithNSString:@"location" withRAREUTJSONObject:patient];
  if (obj == nil) {
    obj = [w getStringWithNSString:@"bv.text.unknown_location"];
  }
  (void) [app putDataWithId:@"pt_location" withId:(NSString *) check_class_cast(obj, [NSString class])];
  ...
  [CCPBVOrderManager patientChanged];
}
...
+ (void)showRelationshipPopupWithRAREWindowViewer:(RAREWindowViewer *)w
  ...
  CCPBVPickList *pl = [[CCPBVPickList alloc] initWithNSString:title withRAREActionLink:link];
  [pl setSupportListDblClickWithBoolean:NO];
  [pl showWithRAREiFunctionCallback:rcb];
}
  ...
  if (self == [CCPBVPatientSelect class]) {
    {
      ...
      CCPBVPatientSelect_noPhotoIcon_ = [[CCPBVPatientSelect_PatientIcon alloc] initWithRAREiPlatformIcon:[RAREPlatform getResourceAsIconWithNSString:@"bv.icon.no_photo"]];
    }
  }
}
...
- (void)copyAllFieldsTo:(CCPBVPatientSelect *)other {
  ...
  other->alwaysShowSearchFirst_ = alwaysShowSearchFirst_;
  ...
  other->genderSearchSupported_ = genderSearchSupported_;
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    ...
    { "populatePreviewWithRARERenderableDataItem:", NULL, "V", 0x4, NULL },
    ...
    { "updateSignalIconWithRARERenderableDataItem:", NULL, "V", 0x2, NULL },
    ...
    { "getLinkedValueWithNSString:withRAREUTJSONObject:", NULL, "LNSObject", 0xc, NULL },
    { "getThimbnailWithRAREaWidget:withNSString:", NULL, "LRAREiPlatformIcon", 0xc, NULL },
  ...
  static J2ObjcFieldInfo fields[] = {
    ...
    { "noPhotoIcon_", NULL, 0x9, "LCCPBVPatientSelect_PatientIcon" },
    ...
    { "pager_", NULL, 0x0, "LCCPBVListPager" },
    { "searchPageSize_", NULL, 0x0, "I" },
    { "psActionListener_", NULL, 0x0, "LCCPBVPatientSelect_CardPatientSelectionActionListener" },
  };
  static J2ObjcClassInfo _CCPBVPatientSelect = { "PatientSelect", "com.sparseware.bellavista", NULL, 0x1, 20, methods, 34, fields, 0, NULL};
...
- (void)actionPerformedWithRAREActionEvent:(RAREActionEvent *)e {
  ...
  NSString *id_ = (NSString *) check_class_cast([((RARERenderableDataItem *) nil_chk([((RARERenderableDataItem *) nil_chk(row)) getWithInt:0])) getValue], [NSString class]);
  id<RAREiPlatformIcon> icon = (id<RAREiPlatformIcon>) check_protocol_cast([((RARERenderableDataItem *) nil_chk([row getWithInt:0])) getIcon], @protocol(RAREiPlatformIcon));
  id<RAREiFunctionCallback> cb = [[CCPBVPatientSelect_CardPatientSelectionActionListener_$1 alloc] initWithRAREiPlatformIcon:icon];
  [CCPBVPatientSelect loadPatientExWithRAREiWidget:w withNSString:id_ withRAREiFunctionCallback:cb];
}
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcFieldInfo fields[] = {
    { "val$icon_", NULL, 0x1012, "LRAREiPlatformIcon" },
  };
  static J2ObjcClassInfo _CCPBVPatientSelect_CardPatientSelectionActionListener_$1 = { "$1", "com.sparseware.bellavista", "PatientSelect$CardPatientSelectionActionListener", 0x8000, 0, NULL, 1, fields, 0, NULL};
...
@end
@implementation CCPBVPatientSelect_PatientIcon

static int CCPBVPatientSelect_PatientIcon_iconSize_;

+ (int)iconSize {
  return CCPBVPatientSelect_PatientIcon_iconSize_;
}

- (id)initWithRAREiPlatformIcon:(id<RAREiPlatformIcon>)icon {
  if (self = [super init]) {
    self->icon_ = icon;
    self->imageIcon_ = [(id) icon isKindOfClass:[RAREUIImageIcon class]];
  }
  return self;
}
...
- (RAREUIImage *)getImage {
  return imageIcon_ ? [((RAREUIImageIcon *) check_class_cast(icon_, [RAREUIImageIcon class])) getImage] : nil;
}
...
- (void)paintWithRAREiPlatformGraphics:(id<RAREiPlatformGraphics>)g
  ...
  int d1 = [RAREUIScreen PLATFORM_PIXELS_1];
  int d2 = [RAREUIScreen PLATFORM_PIXELS_2];
  float isize = CCPBVPatientSelect_PatientIcon_iconSize_;
  y += (height - isize) / 2;
  [((id<RAREiPlatformGraphics>) nil_chk(g)) saveState];
  [((RAREUILineBorder *) nil_chk([CCPBVPatientSelect photoBorder])) clipWithRAREiPlatformGraphics:g withFloat:x withFloat:y withFloat:isize withFloat:isize];
  if (imageIcon_) {
    RAREUIImage *img = [((RAREUIImageIcon *) check_class_cast(icon_, [RAREUIImageIcon class])) getImage];
    [g drawImageWithRAREiPlatformImage:img withFloat:x + d1 withFloat:y + d1 withFloat:isize - d2 withFloat:isize - d2];
  }
  else {
    [((id<RAREiPlatformIcon>) nil_chk(icon_)) paintWithRAREiPlatformGraphics:g withFloat:x + d1 withFloat:y + d1 withFloat:isize - d2 withFloat:isize - d2];
  }
  [g restoreState];
  [[CCPBVPatientSelect photoBorder] paintWithRAREiPlatformGraphics:g withFloat:x withFloat:y withFloat:isize withFloat:isize withBoolean:[[CCPBVPatientSelect photoBorder] isPaintLast]];
}

- (BOOL)isImageLoadedWithRAREiImageObserver:(id<RAREiImageObserver>)imageObserver {
  if (imageIcon_) {
    return [((RAREUIImageIcon *) check_class_cast(icon_, [RAREUIImageIcon class])) isImageLoadedWithRAREiImageObserver:imageObserver];
  }
  return YES;
}

+ (void)initialize {
  if (self == [CCPBVPatientSelect_PatientIcon class]) {
    CCPBVPatientSelect_PatientIcon_iconSize_ = [RAREUIScreen platformPixelsWithInt:48];
  }
}
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcFieldInfo fields[] = {
    { "iconSize_", NULL, 0x18, "I" },
    { "icon_", NULL, 0x10, "LRAREiPlatformIcon" },
    { "imageIcon_", NULL, 0x10, "Z" },
  };
  static J2ObjcClassInfo _CCPBVPatientSelect_PatientIcon = { "PatientIcon", "com.sparseware.bellavista", "PatientSelect", 0x8, 3, methods, 3, fields, 0, NULL};
  return &_CCPBVPatientSelect_PatientIcon;
}
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _CCPBVPatientSelect_$3 = { "$3", "com.sparseware.bellavista", "PatientSelect", 0x8000, 0, NULL, 1, fields, 0, NULL};
    ...
    if ([returnValue isKindOfClass:[JavaLangThrowable class]]) {
      ...
      @try {
        ...
        if ([this$0_ areSamePatientsInSameOrderWithJavaUtilList:list withRARETableViewer:table]) {
          int len = [((id<JavaUtilList>) nil_chk(list)) size];
          for (int i = 0; i < len; i++) {
            RARERenderableDataItem *row = [((RARETableViewer *) nil_chk(table)) getWithInt:i];
            RARERenderableDataItem *os = [((RARERenderableDataItem *) nil_chk([list getWithInt:i])) getWithInt:CCPBVPatientSelect_SIGNAL];
            RARERenderableDataItem *s = [((RARERenderableDataItem *) nil_chk(row)) getWithInt:CCPBVPatientSelect_SIGNAL];
            if (![((RARERenderableDataItem *) nil_chk(os)) valueEqualsWithId:s]) {
              (void) [row setWithInt:CCPBVPatientSelect_SIGNAL withId:s];
              [this$0_ updateSignalIconWithRARERenderableDataItem:row];
...
- (void)cancelWithBoolean:(BOOL)canInterrupt {
  [super cancelWithBoolean:canInterrupt];
  if (![self isDone]) {
    [((RAREActionLink *) nil_chk(val$link_)) cancelWithBoolean:YES];
  }
}
...
- (id)compute {
  ...
  @catch (JavaLangException *e) {
    return canceled_ ? nil : e;
  }
}

- (void)finishWithId:(id)result {
  [((RAREWindowViewer *) nil_chk(val$w_)) hideWaitCursor];
  if ((result != nil) && ![((RARETableViewer *) nil_chk(val$table_)) isDisposed]) {
    if ([result isKindOfClass:[JavaLangException class]]) {
      ...
      @try {
        ...
        empty = ([rows size] == 1) && ([rows getWithInt:0] == this$0_->noPatientsFound_);
        ...
        if (![CCPBVUtils isCardStack]) {
          ...
          if (empty) {
            if (sp != nil) {
              ...
              else if ((val$filter_ == nil) && (val$gender_ == nil)) {
                [((RAREaGroupableButton *) check_class_cast([((id<RAREiFormViewer>) nil_chk([sp getFormViewer])) getWidgetWithNSString:@"searchButton"], [RAREaGroupableButton class])) click];
              }
            }
          }
          else {
...
- (void)finishedWithBoolean:(BOOL)canceled
  ...
  if (canceled) {
    ...
    (void) [val$app_ putDataWithId:@"pt_thumbnail" withId:[CCPBVPatientSelect getThimbnailWithRAREaWidget:val$w_ withNSString:photo]];
...
- (id)compute {
  ...
  @try {
    l = [((RAREWindowViewer *) nil_chk(val$w_)) createActionLinkWithId:[NSString stringWithFormat:@"/hub/main/util/patients/select/%@.json", val$id_]];
    NSString *s = [((RAREActionLink *) nil_chk(l)) getContentAsString];
    if ((s != nil) && ([s sequenceLength] > 0)) {
