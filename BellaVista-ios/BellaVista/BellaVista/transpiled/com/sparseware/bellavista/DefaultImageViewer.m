...
- (void)finishWithId:(id)result {
  ...
  if ([((CCPBVDocument_DocumentItem *) nil_chk(val$attachment_)) getItemType] == [CCPBVDocument_DocumentItemTypeEnum IMAGE_SLIDES]) {
    ...
    [((RARECarouselViewer *) nil_chk(slides)) setVisibleWithBoolean:NO];
    RAREImagePaneViewer *pane = (RAREImagePaneViewer *) check_class_cast([v getWidgetWithNSString:@"imageViewer"], [RAREImagePaneViewer class]);
    [((RAREImagePaneViewer *) nil_chk(pane)) setImageWithRAREUIImage:(RAREUIImage *) check_class_cast(oh->value_, [RAREUIImage class])];
  }
...
- (id)compute {
  @try {
    ...
    if ([((CCPBVDocument_DocumentItem *) nil_chk(val$attachment_)) getItemType] == [CCPBVDocument_DocumentItemTypeEnum IMAGE_SLIDES]) {
      ...
      value = [((RAREWindowViewer *) nil_chk(val$w_)) getImageWithNSString:[val$attachment_ getHREF]];
    }
    RARESPOTViewer *cfg = (RARESPOTViewer *) check_class_cast([val$w_ createConfigurationObjectWithRAREActionLink:[((RAREWindowViewer *) nil_chk(val$w_)) createActionLinkWithId:@"/image_viewer.rml"]], [RARESPOTViewer class]);
    return [[RAREUTObjectHolder alloc] initWithId:cfg withId:value];
  }
