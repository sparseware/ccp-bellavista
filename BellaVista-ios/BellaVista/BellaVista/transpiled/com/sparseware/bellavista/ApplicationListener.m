...
@implementation ComSparsewareBellavistaApplicationListener
...
- (void)applicationPausedWithRAREiPlatformAppContext:(id<RAREiPlatformAppContext>)app {
  if (![ComSparsewareBellavistaUtils isShuttingDown]) {
    [ComSparsewareBellavistaUtils applicationPaused];
  }
}

- (void)applicationResumedWithRAREiPlatformAppContext:(id<RAREiPlatformAppContext>)app {
  if (![ComSparsewareBellavistaUtils isShuttingDown]) {
    [ComSparsewareBellavistaUtils applicationResumed];
  }
}
...
- (void)onConfigurationWillChangeWithId:(id)config {
  if (![ComSparsewareBellavistaUtils isShuttingDown] && ![RAREUIScreen isLargeScreen]) {
    [ComSparsewareBellavistaUtils toggleFullScreenWithBoolean:[RAREUIScreen isWiderForConfigurationWithId:config]];
  }
}
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _ComSparsewareBellavistaApplicationListener = { "ApplicationListener", "com.sparseware.bellavista", NULL, 0x1, 1, methods, 0, NULL, 0, NULL};
  return &_ComSparsewareBellavistaApplicationListener;
}
