...
- (id)init {
  if (self = [super initWithNSString:@"Observation"]) {
    uniqueDates_ = [[JavaUtilHashSet alloc] init];
...
- (void)listWithCCPBViHttpConnection:(id<CCPBViHttpConnection>)conn
  ...
  [((JavaUtilHashSet *) nil_chk(uniqueDates_)) clear];
  NSString *from = nil;
  NSString *to = nil;
...
- (void)processEntryExWithRAREUTJSONObject:(RAREUTJSONObject *)entry_
  ...
  do {
    ...
    if (((CCPBVFHIRFHIRServer *) nil_chk(server_))->debug_ && ![@"" isEqual:vitalld] && vital != nil && ![((JavaUtilHashSet *) nil_chk(uniqueDates_)) addWithId:[NSString stringWithFormat:@"%@-%@", date, vitalld]]) {
      vital = [NSString stringWithFormat:@"{fgColor:badData}DUPLICATE: %@", vital];
    }
    if (jw != nil) {
      (void) [jw object];
      if (date != nil) {
...
- (void)copyAllFieldsTo:(CCPBVFHIRVitals *)other {
  ...
  other->uniqueDates_ = uniqueDates_;
}
