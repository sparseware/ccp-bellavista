...
- (void)initialize__WithCCPBVSettings_Server:(CCPBVSettings_Server *)server {
  ...
  if (fhir != nil) {
    ...
    BOOL log = [fhir optBooleanWithNSString:@"logg_connections"];
    if (log) {
    }
  }
...
+ (void)createForCommandLineTesting {
  CCPBVFHIRFHIRServer *server = [[CCPBVFHIRFHIRServer alloc] initWithNSString:@"https://fhir-open-api.smartplatforms.org/"];
...
- (RAREUTJSONObject *)loginWithNSString:(NSString *)username
  ...
  if ([o optStringWithNSString:@"username" withNSString:nil] == nil) {
    (void) [o putWithNSString:@"username" withId:username];
  }
  authHeader_ = basicAuth;
  loggedInUser_ = [self resolveUserInfoWithRAREUTJSONObject:o];
...
- (NSString *)createAuthorizeUrl {
  ...
  (void) [data putWithId:@"aud" withId:[fhir_ optStringWithNSString:@"oauth_aud" withNSString:[fhir_ optStringWithNSString:@"aud"]]];
...
- (RAREUTJSONObject *)resolveUserInfoWithRAREUTJSONObject:(RAREUTJSONObject *)o {
  if (o == nil) {
    if (userinfoUri_ == nil) {
      NSString *s = tokenUri_;
      int n = [((NSString *) nil_chk(s)) lastIndexOf:'/'];
      s = [NSString stringWithFormat:@"%@userinfo", [s substring:0 endIndex:n + 1]];
      userinfoUri_ = s;
    }
    RAREActionLink *l = [((CCPBVFHIRFHIRServer *) nil_chk([CCPBVFHIRFHIRServer getInstance])) createLinkWithNSString:userinfoUri_];
    o = [[RAREUTJSONObject alloc] initWithNSString:[((RAREActionLink *) nil_chk(l)) getContentAsString]];
  }
  if ([((RAREUTJSONObject *) nil_chk(o)) optStringWithNSString:@"username" withNSString:nil] == nil) {
    (void) [o putWithNSString:@"username" withId:[o optStringWithNSString:@"sub"]];
  }
  NSString *s = [o optStringWithNSString:@"name" withNSString:nil];
  if (s == nil) {
    JavaLangStringBuilder *sb = [[JavaLangStringBuilder alloc] init];
    s = [o optStringWithNSString:@"family_name" withNSString:nil];
    if (s != nil) {
      (void) [((JavaLangStringBuilder *) nil_chk([sb appendWithNSString:s])) appendWithNSString:@", "];
    }
    s = [o optStringWithNSString:@"given_name" withNSString:nil];
    if (s != nil) {
      if (s != nil) {
        (void) [sb appendWithNSString:s];
      }
    }
    NSString *m = [o optStringWithNSString:@"middle_name" withNSString:nil];
    if (m != nil) {
      if (s != nil) {
        (void) [((JavaLangStringBuilder *) nil_chk([sb appendWithNSString:@" "])) appendWithNSString:m];
      }
      else {
    ...
    if ([sb sequenceLength] == 0) {
      (void) [sb appendWithNSString:@"Anonymous User"];
    }
    (void) [o putWithNSString:@"name" withId:[sb description]];
  }
  return o;
}
...
- (void)copyAllFieldsTo:(CCPBVFHIRFHIRServer_AuthHandler *)other {
  ...
  other->userinfoUri_ = userinfoUri_;
}
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    ...
    { "resolveUserInfoWithRAREUTJSONObject:", NULL, "LRAREUTJSONObject", 0x4, "RAREUTJSONException;JavaIoIOException" },
    { "errorWithNSString:withJavaLangThrowable:", NULL, "V", 0x0, NULL },
    { "successWithRAREUTJSONObject:", NULL, "V", 0x0, NULL },
  };
  static J2ObjcFieldInfo fields[] = {
    { "userinfoUri_", NULL, 0x4, "LNSString" },
  ...
  static J2ObjcClassInfo _CCPBVFHIRFHIRServer_AuthHandler = { "AuthHandler", "com.sparseware.bellavista.external.fhir", "FHIRServer", 0x8, 8, methods, 14, fields, 0, NULL};
...
- (id)compute {
  @try {
    ...
    if (id_ != nil) {
      ...
      RAREUTJSONObject *oo = [[RAREUTJSONObject alloc] initWithNSString:id_];
      profile = [oo optStringWithNSString:@"profile" withNSString:nil];
      id_ = [this$0_->fhir_ optStringWithNSString:@"oauth_id_token_user_info_key" withNSString:nil];
      if (id_ != nil) {
        user = [oo optJSONObjectWithNSString:id_];
        if (user != nil) {
          (void) [this$0_ resolveUserInfoWithRAREUTJSONObject:user];
        }
      }
    }
    if (user == nil) {
      if (profile == nil) {
        user = [this$0_ resolveUserInfoWithRAREUTJSONObject:nil];
      }
      else {
        link = [((CCPBVFHIRFHIRServer *) nil_chk([CCPBVFHIRFHIRServer getInstance])) createResourceLinkWithNSString:profile withNSStringArray:[IOSObjectArray arrayWithLength:0 type:[IOSClass classWithClass:[NSString class]]]];
        RAREUTJSONObject *oo = [[RAREUTJSONObject alloc] initWithNSString:[((RAREActionLink *) nil_chk(link)) getContentAsString]];
        user = [CCPBVFHIRUsers populateUserWithNSString:[((CCPBVFHIRFHIRServer *) nil_chk([CCPBVFHIRFHIRServer getInstance])) getIDWithNSString:profile withBoolean:NO] withRAREUTJSONObject:oo];
      }
    }
...
- (id)otherArrayElementEncounteredWithNSString:(NSString *)arrayName
  ...
  if ([@"rest/security/extension/extension" isEqual:arrayName]) {
    ...
    else if ([s isEqual:@"userinfo"]) {
      ((CCPBVFHIRFHIRServer_AuthHandler *) nil_chk(this$0_->authHandler_))->userinfoUri_ = [o optStringWithNSString:@"valueUri" withNSString:nil];
    }
  }
  return value;
}
