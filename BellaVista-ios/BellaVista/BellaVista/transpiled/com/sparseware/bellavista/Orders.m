...
static int CCPBVOrders_SIG_COLUMN_ = 9;
static int CCPBVOrders_STATUS_COLUMN_ = 8;
static int CCPBVOrders_SUMMARY_STATUS_COLUMN_ = 1;
static int CCPBVOrders_SIGNED_COLUMN_ = 14;
static int CCPBVOrders_FLAGGED_COLUMN_ = 15;
...
- (id)init {
  if (self = [super initWithNSString:@"orders" withNSString:@"Orders"]) {
    ...
    medicationsTitle_ = [info optStringWithNSString:@"medicationsTitle" withNSString:nil];
    if (medicationsTitle_ == nil) {
      medicationsTitle_ = [RAREPlatform getResourceAsStringWithNSString:@"bv.text.medications"];
    }
    ivsTitle_ = [info optStringWithNSString:@"ivsTitle" withNSString:nil];
    if (ivsTitle_ == nil) {
      ivsTitle_ = [RAREPlatform getResourceAsStringWithNSString:@"bv.text.iv_solutions"];
    }
  }
  return self;
}
...
- (void)actionPerformedWithRAREActionEvent:(RAREActionEvent *)e {
  ...
  if (item != nil) {
    if ([item isEditable]) {
      [self loadOrderCategoryWithRAREiWidget:cw withRARERenderableDataItem:item];
    }
    else {
      [self filterTableWithRARETableViewer:dataTable_ withNSString:(NSString *) check_class_cast([item getLinkedData], [NSString class])];
    }
  }
}

- (void)loadOrderCategoryWithRAREiWidget:(id<RAREiWidget>)cw
              withRARERenderableDataItem:(RARERenderableDataItem *)item {
  RARETableViewer *table = dataTable_;
  RAREWindowViewer *w = [RAREPlatform getWindowViewer];
  NSString *cat = (NSString *) check_class_cast([((RARERenderableDataItem *) nil_chk(item)) getLinkedData], [NSString class]);
  [item setEditableWithBoolean:NO];
  [item setIconWithRAREiPlatformIcon:[RAREPlatform getResourceAsIconWithNSString:@"bv.icon.dash"]];
  if ([(id) cw isKindOfClass:[RAREComboBoxWidget class]]) {
    [((RAREComboBoxWidget *) check_class_cast(cw, [RAREComboBoxWidget class])) rowChangedWithRARERenderableDataItem:item];
  }
  RAREActionLink *link = [CCPBVUtils createLinkWithRAREiWidget:table withNSString:[NSString stringWithFormat:@"/hub/main/orders/category/%@", cat] withBoolean:NO];
  @try {
    RAREaWorkerTask *task = [[CCPBVOrders_$1 alloc] initWithCCPBVOrders:self withRARETableViewer:table withRARERenderableDataItem:item withNSString:cat withRAREiWidget:cw withRAREWindowViewer:w withRAREActionLink:link];
    [w showProgressPopupWithJavaLangCharSequence:[((RAREWindowViewer *) nil_chk(w)) getStringWithNSString:@"bv.text.loading"]];
...
- (void)onFinishedLoadingWithNSString:(NSString *)eventName
  ...
  if ([CCPBVUtils isCardStack]) {
    ...
    [((RARETableViewer *) nil_chk(table)) pageHome];
  }
}
...
- (void)onTableActionWithNSString:(NSString *)eventName
  ...
  @try {
    ...
    id<RAREiContainer> dv = (id<RAREiContainer>) check_protocol_cast([((id<RAREiFormViewer>) nil_chk(fv)) getWidgetWithNSString:@"documentViewer"], @protocol(RAREiContainer));
    if (dv == nil) {
      RAREActionLink *link = [[RAREActionLink alloc] initWithRAREUTiURLResolver:fv withNSString:@"/document_viewer.rml"];
...
- (void)dataParsedWithRAREiWidget:(id<RAREiWidget>)widget
  ...
  RAREWindowViewer *w = [RAREPlatform getWindowViewer];
  (void) [((RAREWindowViewer *) nil_chk(w)) spawnWithNSObjectArray:[IOSObjectArray arrayWithObjects:(id[]){ [[CCPBVOrders_$2 alloc] initWithCCPBVOrders:self withRAREWindowViewer:w withRARETableViewer:table withJavaUtilList:rows] } count:1 type:[IOSClass classWithClass:[NSObject class]]]];
  [w showWaitCursor];
}
...
- (void)processDataWithRARETableViewer:(RARETableViewer *)table
  ...
  RAREWindowViewer *w = [RAREPlatform getWindowViewer];
  RAREUTIntList *toDelete = [[RAREUTIntList alloc] init];
  ...
  for (int i = 0; i < len; i++) {
    ...
    NSString *sig = @"";
    if (rowSize > CCPBVOrders_SIG_COLUMN_) {
    ...
    [((RARERenderableDataItem *) nil_chk(item2)) setValueWithId:sig];
    ...
    if (!cardstack) {
      ...
      if ([((NSString *) nil_chk(s)) sequenceLength] == 0) {
        ...
        else if ([((NSString *) nil_chk(ivsCategoryID_)) isEqual:type]) {
          s = ivsTitle_;
        }
        else {
      ...
      if (![categorySet containsWithId:s]) {
        ...
        if ([((NSString *) nil_chk(orderID)) sequenceLength] == 0) {
          [category setIconWithRAREiPlatformIcon:[RAREPlatform getResourceAsIconWithNSString:@"bv.icon.download"]];
          [category setEditableWithBoolean:YES];
          [toDelete addWithInt:i];
        }
      }
  ...
  BOOL hasUnloadedCategories = toDelete->_length_ > 0;
  if (hasUnloadedCategories) {
    for (int i = toDelete->_length_ - 1; i > -1; i--) {
      (void) [rows removeWithInt:[toDelete getWithInt:i]];
    }
  }
  if (cardstack) {
    dataLoaded_ = YES;
    allOrders_ = rows;
    [RAREPlatform invokeLaterWithJavaLangRunnable:[[CCPBVOrders_$3 alloc] initWithCCPBVOrders:self withRARETableViewer:table]];
  }
  else {
    ...
    if (!hasMeds) {
      int pos = [RARERenderableDataItem findLinkedObjectIndexWithJavaUtilList:categories withId:medsCategoryID_];
      if (pos == -1) {
        [categories addWithInt:0 withId:category = [[RARERenderableDataItem alloc] initWithNSString:medicationsTitle_ withId:medsCategoryID_ withRAREiPlatformIcon:icon]];
        [category setActionListenerWithRAREiActionListener:self];
      }
      else {
        category = [categories getWithInt:pos];
      }
      [allOrders_ addAllWithJavaUtilCollection:[self createNotDataForCategoryWithRARETableViewer:table withRARERenderableDataItem:category]];
    }
    int selectedIndex;
    if ((categorizedMeds_ != nil) || hasMeds) {
      selectedIndex = hasUnloadedCategories ? 0 : 1;
    }
    else {
      selectedIndex = 0;
    }
    dataLoaded_ = YES;
    [RAREPlatform invokeLaterWithJavaLangRunnable:[[CCPBVOrders_$4 alloc] initWithCCPBVOrders:self withRARETableViewer:table withJavaUtilList:categories withInt:selectedIndex withCCPBVActionPath:path]];
  }
}

- (void)processDataExWithRAREiWidget:(id<RAREiWidget>)context
                    withJavaUtilList:(id<JavaUtilList>)rows {
  int len = [((id<JavaUtilList>) nil_chk(rows)) size];
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (orderStatuses != nil) {
      if ([CCPBVOrderManager isDiscontinuedWithNSString:orderID withJavaUtilMap:orderStatuses]) {
        if (strikeThrough == nil) {
          strikeThrough = [((RAREUIFont *) nil_chk([((RAREUIFont *) nil_chk([((id<RAREiWidget>) nil_chk(context)) getFont])) deriveItalic])) deriveStrikethrough];
        }
        istatus |= CCPBVOrders_STOPPED_FLAG;
        [row setFontWithRAREUIFont:strikeThrough];
        NSString *c = [((RAREUTJSONObject *) nil_chk(CCPBVOrders_statusColors_)) optStringWithNSString:@"Discontinued" withNSString:nil];
    ...
    [row setUserStateFlagWithByte:(char) istatus];
    if (statusColor != nil) {
      [row setForegroundWithRAREUIColor:[RAREUIColorHelper getColorWithNSString:statusColor]];
    }
  }
}
...
- (id<JavaUtilList>)createNotDataForCategoryWithRARETableViewer:(RARETableViewer *)table
                                     withRARERenderableDataItem:(RARERenderableDataItem *)item {
  id<JavaUtilList> rows = [[JavaUtilArrayList alloc] initWithInt:1];
  NSString *cat = (NSString *) check_class_cast([((RARERenderableDataItem *) nil_chk(item)) getLinkedData], [NSString class]);
  NSString *name = [item description];
  NSString *msg = [((RAREWindowViewer *) nil_chk([RAREPlatform getWindowViewer])) getStringWithNSString:@"bv.format.no_orders_found_for_category" withNSObjectArray:[IOSObjectArray arrayWithObjects:(id[]){ name } count:1 type:[IOSClass classWithClass:[NSObject class]]]];
  RARERenderableDataItem *row;
  [rows addWithId:row = [self createNoDataRowWithRARETableViewer:table withNSString:msg]];
  [((RARERenderableDataItem *) nil_chk([((RARERenderableDataItem *) nil_chk(row)) getWithInt:0])) setValueWithId:cat];
  [row addMissingColumnsWithInt:CCPBVOrders_CATEGORY_NAME_POSITION + 1];
  [((RARERenderableDataItem *) nil_chk([row getWithInt:CCPBVOrders_CATEGORY_NAME_POSITION])) setValueWithId:[item getValue]];
  rows = [CCPBVUtils groupRowsWithRARETableViewer:table withJavaUtilList:rows withInt:CCPBVOrders_CATEGORY_NAME_POSITION withInt:-1 withBoolean:NO];
  return rows;
}
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    { "loadOrderCategoryWithRAREiWidget:withRARERenderableDataItem:", NULL, "V", 0x4, NULL },
    ...
    { "processDataExWithRAREiWidget:withJavaUtilList:", NULL, "V", 0x4, NULL },
    ...
    { "createNotDataForCategoryWithRARETableViewer:withRARERenderableDataItem:", NULL, "LJavaUtilList", 0x4, NULL },
  ...
  static J2ObjcFieldInfo fields[] = {
    ...
    { "medicationsTitle_", NULL, 0x4, "LNSString" },
    { "ivsTitle_", NULL, 0x4, "LNSString" },
  ...
  static J2ObjcClassInfo _CCPBVOrders = { "Orders", "com.sparseware.bellavista", NULL, 0x1, 15, methods, 69, fields, 0, NULL};
  return &_CCPBVOrders;
}
...
- (void)finishWithId:(id)result {
  @try {
    ...
    if (rows != nil) {
      [this$0_ processDataExWithRAREiWidget:val$table_ withJavaUtilList:rows];
    }
    if ((rows == nil) || [rows isEmpty]) {
      if (rows == nil) {
        rows = [[JavaUtilArrayList alloc] initWithInt:1];
      }
      NSString *msg = [((RAREWindowViewer *) nil_chk([RAREPlatform getWindowViewer])) getStringWithNSString:@"bv.format.no_orders_found_for_category" withNSObjectArray:[IOSObjectArray arrayWithObjects:(id[]){ [((RARERenderableDataItem *) nil_chk(val$item_)) description] } count:1 type:[IOSClass classWithClass:[NSObject class]]]];
      RARERenderableDataItem *row;
      [((id<JavaUtilList>) nil_chk(rows)) addWithId:row = [this$0_ createNoDataRowWithRARETableViewer:val$table_ withNSString:msg]];
      [((RARERenderableDataItem *) nil_chk([((RARERenderableDataItem *) nil_chk(row)) getWithInt:0])) setValueWithId:val$cat_];
      [row addMissingColumnsWithInt:CCPBVOrders_CATEGORY_NAME_POSITION + 1];
      [((RARERenderableDataItem *) nil_chk([row getWithInt:CCPBVOrders_CATEGORY_NAME_POSITION])) setValueWithId:[val$item_ getValue]];
    }
    rows = [CCPBVUtils groupRowsWithRARETableViewer:val$table_ withJavaUtilList:rows withInt:CCPBVOrders_CATEGORY_NAME_POSITION withInt:-1 withBoolean:NO];
    [((id<JavaUtilList>) nil_chk(this$0_->allOrders_)) addAllWithJavaUtilCollection:rows];
    [val$table_ unfilter];
    [val$table_ setAllWithJavaUtilCollection:this$0_->allOrders_];
    [this$0_ filterTableWithRARETableViewer:val$table_ withNSString:val$cat_];
    [((id<RAREiWidget>) nil_chk(val$cw_)) update];
  }
  @finally {
...
- (id)compute {
  @try {
    id<JavaUtilList> rows = [((RARETableViewer *) nil_chk(val$table_)) isDisposed] ? nil : [val$table_ parseDataLinkWithRAREActionLink:val$link_ withBoolean:YES];
    if (![val$table_ isDisposed]) {
    }
    return rows;
  }
  @catch (JavaLangException *e) {
...
withRARERenderableDataItem:(RARERenderableDataItem *)capture$1
     ...
     withRAREWindowViewer:(RAREWindowViewer *)capture$4
       withRAREActionLink:(RAREActionLink *)capture$5 {
  this$0_ = outer$;
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcFieldInfo fields[] = {
    ...
    { "val$item_", NULL, 0x1012, "LRARERenderableDataItem" },
    { "val$cat_", NULL, 0x1012, "LNSString" },
    { "val$cw_", NULL, 0x1012, "LRAREiWidget" },
    { "val$w_", NULL, 0x1012, "LRAREWindowViewer" },
    { "val$link_", NULL, 0x1012, "LRAREActionLink" },
  };
  static J2ObjcClassInfo _CCPBVOrders_$1 = { "$1", "com.sparseware.bellavista", "Orders", 0x8000, 1, methods, 7, fields, 0, NULL};
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _CCPBVOrders_$2 = { "$2", "com.sparseware.bellavista", "Orders", 0x8000, 1, methods, 4, fields, 0, NULL};
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _CCPBVOrders_$3 = { "$3", "com.sparseware.bellavista", "Orders", 0x8000, 0, NULL, 2, fields, 0, NULL};
  return &_CCPBVOrders_$3;
}

@end
@implementation CCPBVOrders_$4
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _CCPBVOrders_$4 = { "$4", "com.sparseware.bellavista", "Orders", 0x8000, 0, NULL, 5, fields, 0, NULL};
  return &_CCPBVOrders_$4;
}
