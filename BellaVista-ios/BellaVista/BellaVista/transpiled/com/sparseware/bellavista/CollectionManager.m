...
- (id)init {
  if (self = [super init]) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      RAREActionLink *link = [CCPBVUtils createLinkWithRAREiWidget:w withNSString:url withBoolean:YES];
...
- (void)updateUIWithRAREWindowViewer:(RAREWindowViewer *)w
  ...
  if (len == 1) {
    id<JavaUtilCollection> c = [dc getCollectionWithRAREiWidget:w];
    id<JavaUtilIterator> it = [((id<JavaUtilCollection>) nil_chk(c)) iterator];
    RARERenderableDataItem *item = [((id<JavaUtilIterator>) nil_chk(it)) next];
    if (![((RARERenderableDataItem *) nil_chk(item)) isEnabled]) {
      len = 0;
    }
  }
