...
- (id)init {
  if (self = [super initWithNSString:@"labs" withNSString:@"Labs"]) {
    ...
    if (trendPanels_ != nil) {
      ...
      [CCPBVTrendPanel setupLabelsWithRAREWindowViewer:[RAREPlatform getWindowViewer]];
    }
...
- (void)filterTableWithRARERenderableDataItem:(RARERenderableDataItem *)filterItem {
  if (filterItem != nil) {
    ...
    if (all) {
      ...
      [table filterWithRAREUTiFilter:[[CCPBVLabs_$1 alloc] initWithCCPBVLabs:self withNSString:cat]];
      if (![table isEmpty]) {
        [table scrollRowToTopWithInt:0];
      }
    }
...
- (BOOL)checkRowWithRARERenderableDataItem:(RARERenderableDataItem *)row
  ...
  @try {
    do {
      ...
      if (!isSummary_) {
        if ([CCPBVUtils isChartableWithNSString:value]) {
...
- (void)onLabReportLoadWithNSString:(NSString *)eventName
  ...
  CCPBVDocument *doc = (CCPBVDocument *) check_class_cast([((RAREGridPaneViewer *) nil_chk(gp)) getLinkedData], [CCPBVDocument class]);
  if (doc != nil) {
...
- (CCPBVTrendPanel *)createTrendPanelFromTableWithRARETableViewer:(RARETableViewer *)table
                                                     withNSString:(NSString *)title {
  CCPBVTrendPanel *p = [[CCPBVTrendPanel alloc] initWithNSString:title withNSString:title withRAREUTJSONArray:nil];
  ...
  for (int i = 0; i < len; i++) {
    ...
    for (int n = 0; n < nlen; n++) {
      ...
      if ([CCPBVUtils isChartableWithNSString:value]) {
        ...
        [((RAREUTMutableInteger *) nil_chk(count)) incrementAndGet];
        [p addTrendWithNSString:key withJavaUtilDate:(JavaUtilDate *) check_class_cast([((RARERenderableDataItem *) nil_chk([row getWithInt:[CCPBVaResultsManager DATE_POSITION]])) getValue], [JavaUtilDate class]) withNSString:(NSString *) check_class_cast([name getValue], [NSString class]) withRARERenderableDataItem:valueItem];
      }
    }
  }
  return p;
...
- (IOSObjectArray *)createTrendPanelsWithRAREUTJSONArray:(RAREUTJSONArray *)trends {
  ...
  IOSObjectArray *a = [IOSObjectArray arrayWithLength:len type:[IOSClass classWithClass:[CCPBVTrendPanel class]]];
  for (int i = 0; i < len; i++) {
    RAREUTJSONObject *o = [((RAREUTJSONArray *) nil_chk(trends)) getJSONObjectWithInt:i];
    CCPBVTrendPanel *p = [[CCPBVTrendPanel alloc] initWithNSString:[((RAREUTJSONObject *) nil_chk(o)) getStringWithNSString:@"name"] withNSString:[o getStringWithNSString:@"title"] withRAREUTJSONArray:[o getJSONArrayWithNSString:@"keys"]];
...
- (void)loadStainsWithRAREWindowViewer:(RAREWindowViewer *)w
  ...
  int len = [((CCPBVDocument *) nil_chk(doc)) getAttachmentCount];
  BOOL error = NO;
  for (int i = 0; i < len; i++) {
    CCPBVDocument_DocumentItem *di = [doc getAttachmentWithInt:i];
    if (![@"stains" isEqual:((CCPBVDocument_DocumentItem *) nil_chk(di))->customItemType_]) {
      continue;
    }
    @try {
...
- (void)loadSusceptibilitiesWithRAREWindowViewer:(RAREWindowViewer *)w
  ...
  for (int i = 0; i < len; i++) {
    CCPBVDocument_DocumentItem *di = [doc getAttachmentWithInt:i];
    if (![@"susceptibilities" isEqual:((CCPBVDocument_DocumentItem *) nil_chk(di))->customItemType_]) {
      continue;
    }
    @try {
...
- (BOOL)reportHasStainsWithCCPBVDocument:(CCPBVDocument *)doc {
  ...
  for (int i = 0; i < len; i++) {
    CCPBVDocument_DocumentItem *di = [doc getAttachmentWithInt:i];
    if ([@"stains" equalsIgnoreCase:((CCPBVDocument_DocumentItem *) nil_chk(di))->customItemType_]) {
      return YES;
    }
  }
  return NO;
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    ...
    { "createTrendPanelFromTableWithRARETableViewer:withNSString:", NULL, "LCCPBVTrendPanel", 0x4, NULL },
  ...
  static J2ObjcClassInfo _CCPBVLabs = { "Labs", "com.sparseware.bellavista", NULL, 0x1, 19, methods, 26, fields, 0, NULL};
  return &_CCPBVLabs;
}
...
withRAREUTiStringConverter:(id<RAREUTiStringConverter>)converter {
  ...
  if (this$0_->currentView_ == [CCPBVResultsViewEnum SPREADSHEET]) {
    ...
    item = [((RARERenderableDataItem *) nil_chk(item)) getItemExWithInt:[CCPBVLabs CATEGORY_NAME_POSITION]];
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _CCPBVLabs_$1 = { "$1", "com.sparseware.bellavista", "Labs", 0x8000, 1, methods, 2, fields, 0, NULL};
