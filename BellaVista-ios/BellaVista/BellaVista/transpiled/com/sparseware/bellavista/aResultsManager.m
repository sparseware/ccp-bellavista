...
@implementation ComSparsewareBellavistaaResultsManager

static int ComSparsewareBellavistaaResultsManager_DATE_POSITION_ = 0;
static int ComSparsewareBellavistaaResultsManager_NAME_POSITION_ = 1;
static int ComSparsewareBellavistaaResultsManager_VALUE_POSITION_ = 2;
static int ComSparsewareBellavistaaResultsManager_RANGE_POSITION_ = -1;
static int ComSparsewareBellavistaaResultsManager_MIN_ANGLED_LABEL_HEIGHT_ = 400;
static int ComSparsewareBellavistaaResultsManager_MIN_POINTSLABEL_HEIGHT_ = 400;

+ (int)DATE_POSITION {
  return ComSparsewareBellavistaaResultsManager_DATE_POSITION_;
}
...
+ (int)NAME_POSITION {
  return ComSparsewareBellavistaaResultsManager_NAME_POSITION_;
}
...
+ (int)VALUE_POSITION {
  return ComSparsewareBellavistaaResultsManager_VALUE_POSITION_;
}
...
+ (int)RANGE_POSITION {
  return ComSparsewareBellavistaaResultsManager_RANGE_POSITION_;
}
...
+ (int)MIN_ANGLED_LABEL_HEIGHT {
  return ComSparsewareBellavistaaResultsManager_MIN_ANGLED_LABEL_HEIGHT_;
}
...
+ (int)MIN_POINTSLABEL_HEIGHT {
  return ComSparsewareBellavistaaResultsManager_MIN_POINTSLABEL_HEIGHT_;
}
...
- (ComSparsewareBellavistaActionPath *)getDisplayedActionPath {
  ComSparsewareBellavistaActionPath *path = [[ComSparsewareBellavistaActionPath alloc] initWithNSStringArray:[IOSObjectArray arrayWithObjects:(id[]){ [ComSparsewareBellavistaUtils getPatientID], namePrefix_, [((NSString *) nil_chk([((ComSparsewareBellavistaResultsViewEnum *) nil_chk(currentView_)) description])) lowercaseStringWithJRELocale:[JavaUtilLocale US]] } count:3 type:[IOSClass classWithClass:[NSString class]]]];
  [self addCurrentPathIDWithComSparsewareBellavistaActionPath:path];
  return path;
}

- (void)handleActionPathWithComSparsewareBellavistaActionPath:(ComSparsewareBellavistaActionPath *)path {
  NSString *view = [((ComSparsewareBellavistaActionPath *) nil_chk(path)) shift];
  if (view != nil) {
    @try {
      keyPath_ = path;
...
- (void)onChartResizeWithNSString:(NSString *)eventName
                  withRAREiWidget:(id<RAREiWidget>)widget
          withJavaUtilEventObject:(JavaUtilEventObject *)event {
  [((ComSparsewareBellavistaaChartHandler *) nil_chk(chartHandler_)) adjustForSizeWithRAREChartViewer:(RAREChartViewer *) check_class_cast(widget, [RAREChartViewer class])];
}
...
- (void)spreadsheetRowCreatedWithRARETableViewer:(RARETableViewer *)table
  ...
  if (sourceRow != nil && ComSparsewareBellavistaaResultsManager_RANGE_POSITION_ > -1) {
    (void) [spreadsheetRow setItemWithInt:[((RARERenderableDataItem *) nil_chk(spreadsheetRow)) size] - 1 withRARERenderableDataItem:[sourceRow getWithInt:ComSparsewareBellavistaaResultsManager_RANGE_POSITION_]];
  }
}
...
- (void)updateSpreadsheetRowsWithRARETableViewer:(RARETableViewer *)table {
  ...
  @try {
    ...
    for (int i = 0; i < len; i++) {
      ...
      test = [((RARERenderableDataItem *) nil_chk(orow)) getWithInt:ComSparsewareBellavistaaResultsManager_NAME_POSITION_];
      ...
      JavaUtilDate *date = (JavaUtilDate *) check_class_cast([((RARERenderableDataItem *) nil_chk([orow getWithInt:ComSparsewareBellavistaaResultsManager_DATE_POSITION_])) getValue], [JavaUtilDate class]);
      ...
      for (int col = 0; col < clen; col++) {
        if ([((JavaUtilDate *) IOSObjectArray_Get(dates, col + start)) getTime] == time) {
          found = YES;
          (void) [((RARERenderableDataItem *) nil_chk(row)) setItemWithInt:col + 1 withRARERenderableDataItem:[orow getWithInt:ComSparsewareBellavistaaResultsManager_VALUE_POSITION_]];
        }
      }
      if (!found) {
    ...
    if (categorize) {
      ...
      RAREUIFont *bold = [ComSparsewareBellavistaUtils getListWidgetBoldFontWithRAREiWidget:table];
    ...
    id<JavaLangRunnable> r = [[ComSparsewareBellavistaaResultsManager_$1 alloc] initWithComSparsewareBellavistaaResultsManager:self withRARETableViewer:table withRAREWindowViewer:w];
...
- (void)clearChartsWithRARETableViewer:(RARETableViewer *)table {
  ...
  if (stack != nil) {
    ...
    [stack update];
    [((ComSparsewareBellavistaaChartHandler *) nil_chk(chartHandler_)) updateZoomButtonsWithRAREiContainer:[stack getFormViewer]];
  }
  else {
    RARESplitPaneViewer *sp = [ComSparsewareBellavistaUtils getSplitPaneViewerWithRAREiWidget:table];
    if (sp != nil) {
...
- (void)changeViewWithNSString:(NSString *)name
               withRAREiWidget:(id<RAREiWidget>)widget {
  ComSparsewareBellavistaResultsViewEnum *view = [ComSparsewareBellavistaResultsViewEnum valueOfWithNSString:[((NSString *) nil_chk(name)) uppercaseStringWithJRELocale:[JavaUtilLocale US]]];
  if (view == currentView_) {
    return;
  }
  if (view == [ComSparsewareBellavistaResultsViewEnum SPREADSHEET] && chartHandler_ != nil) {
    [chartHandler_ resetChartPoints];
  }
  id<RAREiContainer> parent = [((id<RAREiWidget>) nil_chk(widget)) getFormViewer];
  selectionChecked_ = NO;
  switch ([view ordinal]) {
    case ComSparsewareBellavistaResultsView_TRENDS:
    currentView_ = view;
    ...
    break;
    default:
    [self changeViewExWithComSparsewareBellavistaResultsViewEnum:view];
    break;
  }
}
...
- (void)showChartForSelectedItemExWithRARETableViewer:(RARETableViewer *)table
  ...
  NSString *key;
  BOOL spreadsheet = currentView_ == [ComSparsewareBellavistaResultsViewEnum SPREADSHEET];
  if (spreadsheet) {
    ...
    key = (NSString *) check_class_cast([((RARERenderableDataItem *) nil_chk([((RARERenderableDataItem *) nil_chk(row)) getWithInt:ComSparsewareBellavistaaResultsManager_NAME_POSITION_])) getLinkedData], [NSString class]);
  }
  if (key == nil) {
    return;
  }
  RAREaWorkerTask *task = [[ComSparsewareBellavistaaResultsManager_$2 alloc] initWithComSparsewareBellavistaaResultsManager:self withNSString:key withRAREWindowViewer:w withRARETableViewer:table withRAREStackPaneViewer:sp withJavaLangBoolean:forward withBoolean:horizontal];
...
- (void)selectTableIndexWithRARETableViewer:(RARETableViewer *)table
  ...
  if (![((RARETableViewer *) nil_chk(table)) isDisposed]) {
    ...
    [RAREPlatform invokeLaterWithJavaLangRunnable:[[ComSparsewareBellavistaaResultsManager_$3 alloc] initWithRARETableViewer:table withInt:index]];
  }
}
...
- (void)updateSpreadsheetColumnsWithRARETableViewer:(RARETableViewer *)table {
  ...
  int len = [((RARETableViewer *) nil_chk(table)) getColumnCount] - ComSparsewareBellavistaaResultsManager_RANGE_POSITION_ > -1 ? 1 : 0;
...
- (void)changePageWithRAREiWidget:(id<RAREiWidget>)widget
  ...
  if ([((NSString *) nil_chk([((id<RAREiContainer>) nil_chk([((id<RAREiWidget>) nil_chk(widget)) getParent])) getName])) isEqual:@"tableToolbar"]) {
    ...
    (void) [((id<RAREiPlatformAppContext>) nil_chk([RAREPlatform getAppContext])) executeBackgroundTaskWithJavaLangRunnable:[[ComSparsewareBellavistaaResultsManager_$4 alloc] initWithComSparsewareBellavistaaResultsManager:self withRARETableViewer:table]];
  }
}
...
- (void)showSpreesheetWithRAREiContainer:(id<RAREiContainer>)fv {
  ...
  [w showWaitCursor];
  (void) [((id<RAREiPlatformAppContext>) nil_chk([RAREPlatform getAppContext])) executeBackgroundTaskWithJavaLangRunnable:[[ComSparsewareBellavistaaResultsManager_$5 alloc] initWithComSparsewareBellavistaaResultsManager:self withRARETableViewer:table]];
}
  ...
  @try {
    ...
    if (trends) {
      ...
      if (sp == nil) {
        ...
        [w activateViewerWithId:url withNSString:[((id<RAREiTarget>) nil_chk([sp getRegionWithInt:1])) getName]];
      }
    }
    else {
      if (sp != nil) {
        ...
        if (v != nil && [((NSString *) nil_chk([v getName])) isEqual:[NSString stringWithFormat:@"%@Charts", namePrefix_]]) {
          ...
          if (key == nil) {
            ...
            [self handlePathKeyWithRARETableViewer:table withNSString:key withInt:1 withBoolean:YES];
          }
        }
        else {
          if (chartHandler_ != nil) {
          ...
          [w activateViewerWithId:[NSString stringWithFormat:@"%@_charts.rml", namePrefix_] withNSString:[((id<RAREiTarget>) nil_chk([sp getRegionWithInt:1])) getName]];
        }
      }
    }
...
- (NSString *)getFirstChartableKey {
  ...
  if (spreadsheet) {
    ...
    return (NSString *) check_class_cast([((RARERenderableDataItem *) nil_chk([((RARERenderableDataItem *) nil_chk(row)) getWithInt:ComSparsewareBellavistaaResultsManager_NAME_POSITION_])) getLinkedData], [NSString class]);
  }
}
...
- (void)onDisposeWithNSString:(NSString *)eventName
              withRAREiWidget:(id<RAREiWidget>)widget
      withJavaUtilEventObject:(JavaUtilEventObject *)event {
  [ComSparsewareBellavistaUtils removeActionPathSupporterWithComSparsewareBellavistaActionPath_iActionPathSupporter:self];
  [self reset];
}
...
- (void)onTableCreatedWithNSString:(NSString *)eventName
           ...
           withJavaUtilEventObject:(JavaUtilEventObject *)event {
  [self reset];
  [ComSparsewareBellavistaUtils addActionPathSupporterWithComSparsewareBellavistaActionPath_iActionPathSupporter:self];
...
- (void)slideToChartableItemWithBoolean:(BOOL)next
  ...
  if (pos == -1) {
    [ComSparsewareBellavistaUtils showPullBackAnimationWithRAREiViewer:[((RAREStackPaneViewer *) nil_chk(sp)) getActiveViewer] withBoolean:horizontal withBoolean:next ? NO : YES];
  }
  else {
...
- (BOOL)isChartableWithRARERenderableDataItem:(RARERenderableDataItem *)row {
  ...
  NSString *key = (NSString *) check_class_cast([((RARERenderableDataItem *) nil_chk([((RARERenderableDataItem *) nil_chk(row)) getWithInt:ComSparsewareBellavistaaResultsManager_NAME_POSITION_])) getLinkedData], [NSString class]);
  if (key == nil) {
    return NO;
  }
  ...
  NSString *s = [((NSString *) nil_chk([((RARERenderableDataItem *) nil_chk([row getWithInt:ComSparsewareBellavistaaResultsManager_VALUE_POSITION_])) description])) trim];
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    ...
    { "handleActionPathExWithComSparsewareBellavistaActionPath:", NULL, "V", 0x4, NULL },
    { "addCurrentPathIDWithComSparsewareBellavistaActionPath:", NULL, "V", 0x4, NULL },
    ...
    { "changeViewExWithComSparsewareBellavistaResultsViewEnum:", NULL, "V", 0x4, NULL },
  ...
  static J2ObjcFieldInfo fields[] = {
    ...
    { "currentView_", NULL, 0x4, "LComSparsewareBellavistaResultsViewEnum" },
    ...
    { "chartHandler_", NULL, 0x4, "LComSparsewareBellavistaaChartHandler" },
    ...
    { "keyPath_", NULL, 0x4, "LComSparsewareBellavistaActionPath" },
  ...
  static J2ObjcClassInfo _ComSparsewareBellavistaaResultsManager = { "aResultsManager", "com.sparseware.bellavista", NULL, 0x401, 29, methods, 26, fields, 0, NULL};
  return &_ComSparsewareBellavistaaResultsManager;
}

@end
@implementation ComSparsewareBellavistaaResultsManager_$1
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcFieldInfo fields[] = {
    { "this$0_", NULL, 0x1012, "LComSparsewareBellavistaaResultsManager" },
    { "val$table_", NULL, 0x1012, "LRARETableViewer" },
    { "val$w_", NULL, 0x1012, "LRAREWindowViewer" },
  };
  static J2ObjcClassInfo _ComSparsewareBellavistaaResultsManager_$1 = { "$1", "com.sparseware.bellavista", "aResultsManager", 0x8000, 0, NULL, 3, fields, 0, NULL};
  return &_ComSparsewareBellavistaaResultsManager_$1;
}

@end
@implementation ComSparsewareBellavistaaResultsManager_$2

- (id)compute {
  @try {
    return [((ComSparsewareBellavistaaChartHandler *) nil_chk(this$0_->chartHandler_)) createSeriesWithJavaUtilList:this$0_->originalRows_ withNSString:val$key_ withInt:[ComSparsewareBellavistaaResultsManager RANGE_POSITION]];
  }
  @catch (JavaLangException *e) {
    return e;
  }
...
- (void)finishWithId:(id)result {
  ...
  RAREChartViewer *cv = [((ComSparsewareBellavistaaChartHandler *) nil_chk(this$0_->chartHandler_)) createChartWithRAREiFormViewer:[((RAREStackPaneViewer *) nil_chk(val$sp_)) getFormViewer] withNSString:val$key_ withInt:1 withRAREChartDataItem:series];
  id<RAREiTransitionAnimator> ta = val$forward_ == nil ? nil : this$0_->transitionAnimation_;
  [ComSparsewareBellavistaUtils setViewerInStackPaneViewerWithRAREStackPaneViewer:val$sp_ withRAREiViewer:cv withRAREiTransitionAnimator:ta withBoolean:val$forward_ == nil ? YES : [val$forward_ booleanValue] withBoolean:val$horizontal_ withBoolean:YES];
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _ComSparsewareBellavistaaResultsManager_$2 = { "$2", "com.sparseware.bellavista", "aResultsManager", 0x8000, 1, methods, 7, fields, 0, NULL};
  return &_ComSparsewareBellavistaaResultsManager_$2;
}

@end
@implementation ComSparsewareBellavistaaResultsManager_$3
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _ComSparsewareBellavistaaResultsManager_$3 = { "$3", "com.sparseware.bellavista", "aResultsManager", 0x8000, 0, NULL, 2, fields, 0, NULL};
  return &_ComSparsewareBellavistaaResultsManager_$3;
}

@end
@implementation ComSparsewareBellavistaaResultsManager_$4
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _ComSparsewareBellavistaaResultsManager_$4 = { "$4", "com.sparseware.bellavista", "aResultsManager", 0x8000, 0, NULL, 2, fields, 0, NULL};
  return &_ComSparsewareBellavistaaResultsManager_$4;
}

@end
@implementation ComSparsewareBellavistaaResultsManager_$5
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _ComSparsewareBellavistaaResultsManager_$5 = { "$5", "com.sparseware.bellavista", "aResultsManager", 0x8000, 0, NULL, 2, fields, 0, NULL};
  return &_ComSparsewareBellavistaaResultsManager_$5;
}
