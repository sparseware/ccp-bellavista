...
public class Procedures extends Notes {
  ...
  public Procedures() {
    ...
    JSONObject info = (JSONObject) Platform.getAppContext().getData("proceduresInfo");

    attachmentColumn = info.optInt("attachmentColumn", -1);
    parentColumn     = info.optInt("parentColumn", -1);
    infoName         = "proceduresInfo";
  ...
  /**
   * Populates the card stack form with recent procedures and their status
   *
   * @param fv
   *          the form container
   * @param rows
   *          the rows containing the procedures
   */
  ...
  protected class OrdersStackActionListener implements iActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      StackPaneViewer sp = CardStackUtils.createListItemsOrPageViewer(null, dataTable.getFormViewer(),
                             dataTable.getRawRows(), -1, 1, null, false, true);
