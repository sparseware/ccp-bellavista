...
public class Summary extends aEventHandler {
   ...
   *          screen
   */
  public void calculateRangesAndUpdateUI(iFormViewer fv, ChartViewer chart, List<RenderableDataItem> rows,
          boolean summary) {
    int len = rows.size();
    ...
    JSONObject combo     = chartInfo.getJSONObject(summary
            ? "summaryCombo"
            : "combo");
  ...
  public void updateForm(iContainer fv, Map<String, VitalRange> vitals, Date beg, Date end, HashSet chartVitals) {
    ...
    if (Utils.isCardStack()) {
      String labs = w.getString(hasRecentLabs
                                ? "bv.text.summary_has_recent_labs"
                                : "bv.text.summary_no_recent_labs");
  ...
  public void onConfigureCardStack(String eventName, iWidget widget, EventObject event) {
    ...
    if ((pb != null) &&!pb.isEnabled()) {
      pb.setText(Platform.getResourceAsString(pb.isEnabled()
              ? "bv.text.has_flags"
              : "bv.text.no_flags"));
    ...
    if (pb != null) {
      pb.setText(Platform.getResourceAsString(pb.isEnabled()
              ? "bv.text.has_allergies"
              : "bv.text.no_known_allergies"));
