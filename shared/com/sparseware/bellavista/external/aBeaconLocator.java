...
public abstract class aBeaconLocator extends aPatientLocator implements iChangeListener {
  ...
  public void stateChanged(EventObject e) {
    ...
    if (list != null) {
      ...
      LocatorChangeEvent lce = new LocatorChangeEvent(this, list == nearbyPatients ? LocatorChangeType.PATIENTS
   ...
   */
  protected void resolveBeacons(final String url, final HashMap<String, Beacon> beacons, final List<RenderableDataItem> list,
      final int col, final iFunctionCallback cb) {
  ...
  protected void synchronizeItemsWithBeacons(List<RenderableDataItem> items, HashMap<String, Beacon> beacons, int col) {
    ...
    for (int i = len - 1; i > -1; i--) {
      RenderableDataItem item = items.get(i).get(col);
      String id = (String) item.getLinkedData();
      Beacon b = id == null ? null : beacons.get(id);
      if (b == null) {
        items.remove(i);
      } else {
  ...
  protected void updateBeaconIsForDemo(final HashMap<String, Beacon> beacons, final List<RenderableDataItem> list, int col) {
    Iterator<String> it = beacons.keySet().iterator();
    int len = list.size();
    while (it.hasNext()) {
      String bid = it.next();
  ...
  public static class Beacon {
    ...
    public Beacon(String bstring) {
      ...
      uuid = n == -1 ? bstring : bstring.substring(0, n);
      if (n != -1) {
